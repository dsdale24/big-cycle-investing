...
**Merge method — always squash-merge.** Every PR to `main` is merged using **squash-merge** (`gh pr merge N --squash --body ...` or the GitHub UI's squash option) — regardless of branch prefix or whether the diff crosses the coordinator deny list. This is enforced mechanically: branch protection on `main` sets `required_linear_history: true` (per #111), which blocks merge commits outright — `gh pr merge N --merge` fails with `Merge commits are not allowed on this repository`. Squash also serves the original intent: it collapses per-PR iteration history (CHANGES-REQUIRED back-and-forth, nit cleanups, testing-agent re-runs after coding-agent fixes) into one tidy commit on `main`. The PR thread retains the full iteration history for audit; `main`'s log stays a clean record of landed units of work. The squashed commit's body MUST include the final `## Pre-merge review` block.
